...
Tell the agent to merge its own branch via `/wm:merge`. This lets the agent handle
rebasing and conflict resolution.

```bash
# Tell agent to commit, rebase, and merge
workmux send agent-a "/wm:merge"
...
workmux send auth-module "/wm:merge"
workmux wait auth-module --timeout 120
workmux send api-tests "/wm:merge"
...
workmux send docs-update "/wm:merge"
```
...
5. **Merge one at a time** by sending `/wm:merge` to each agent sequentially. Wait
